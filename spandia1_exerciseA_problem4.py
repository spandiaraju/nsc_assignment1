...
def track_roi_fluorescence(frames, mask, type="mean"):
    """
    Track the average fluorescence of an ROI over time.

    Parameters:
    - frames: A 3D numpy array with dimensions (time, height, width).
    - mask: A 2D numpy array with dimensions (height, width) that represents the ROI.

    Returns:
    - A 1D numpy array representing the time series of average fluorescence within the ROI.
    """
    time_series = []
...
def main():
   ...
   for i in range(5):
       plt.plot(all_average_fluorescence_time_series[i], label = "Average Fluorescence")
       plt.plot(all_max_fluorescence_time_series[i], label = "Max Fluorescence")
